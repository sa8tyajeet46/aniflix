...
function InfoModal({visible,onClose}:InfoModalInterface) {
  ...
  return (
    <div className='absolute top-1/2 bg-zinc-600 text-white   z-50 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-md  w-auto'>
        <video src={data?.movie?.videoUrl} autoPlay muted className="h-[25vh] rounded-md relative"></video>
        <div className='absolute top-2 right-2 text-white cursor-pointer' onClick={handleClose}>
        ...
        <div className=" flex flex-col text-white py-2 px-3 space-y-0.5">
          ...
          <div className="w-full flex text-2xl font-semibold">{data?.movie?.title}</div>
          <div className="w-full flex">{data?.movie?.duration}</div>
          {/* <div className="w-full flex">{data?.movie?.genre}</div> */}
          <div className=" flex w-[400px]">{data?.movie?.description}</div>
        </div>
        {/* <div>
